use crate::{
    canvas::{Canvas, WeightedCanvas},
    geometry::{distance, nearest_pixel, Line, OrderedPolygon, Pixel, UnorderedPolygon},
};
use std::path::Path;
use voronoi::Point;
...
pub fn weighted_raster_centroid(poly: &OrderedPolygon, weights: &mut WeightedCanvas) -> Point {
...
pub fn polygon_raster_bbox(poly: &OrderedPolygon) -> [[i32; 2]; 2] {
...
pub fn scanline_rasterize_polygon(poly: &OrderedPolygon, color: [f32; 3], canvas: &mut Canvas) {
...
pub fn scanline_nodes(poly: &OrderedPolygon, scan_y: f64, width: f64) -> Vec<Pixel> {
...
pub fn color_sampled_voronoi(
    path: &Path,
    cells: Vec<Vec<Point>>,
    canvas: &mut Canvas,
    weights: &mut WeightedCanvas,
) {
    let mut sorted;
    let mut color;
    let color_canvas = Canvas::from_image(path);
    let (mut centroid, mut cx, mut cy);
    for face in cells {
        sorted = UnorderedPolygon::from_face(&face).sort();
